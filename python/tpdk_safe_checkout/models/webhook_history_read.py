...
class WebhookHistoryRead(BaseModel):
    ...
    def event_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['offer.transaction.authorized', 'offer.transaction.accepted', 'offer.transaction.shipped', 'offer.transaction.delivered', 'offer.transaction.completed', 'offer.transaction.canceled', 'offer.transaction.disputed', 'offer.closed', 'persona.added', 'offer.transaction.reviewed']):
            raise ValueError("must be one of enum values ('offer.transaction.authorized', 'offer.transaction.accepted', 'offer.transaction.shipped', 'offer.transaction.delivered', 'offer.transaction.completed', 'offer.transaction.canceled', 'offer.transaction.disputed', 'offer.closed', 'persona.added', 'offer.transaction.reviewed')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # and model_fields_set contains the field
        if self.normalized_object is None and "normalized_object" in self.model_fields_set:
